...
def historic_sma_cross(historic_short_sma,
    ...
    crossing_buy = ((historic_short_sma > historic_long_sma) & (previous_historic_short_sma < previous_historic_long_sma) & (historic_last_price > historic_long_sma))
    crossing_sell = ((historic_short_sma < historic_long_sma) & (previous_historic_short_sma > previous_historic_long_sma) & (historic_last_price < historic_long_sma))
    if crossing_sell:
        print(figi, 'SELL', date)
    if crossing_buy:
        print(figi, 'BUY', date)


def calc_one_historic_signal(number, df_fin_close_prices):
    df_all_historic_sma = pd.read_csv('SMA.csv', sep=';', index_col=0)
    for i in df_all_historic_sma.columns[::2]:
        ...
        if df_historic_short_sma.size != 0:  # проверка на пустой DF
            historic_short_sma = df_historic_short_sma.loc[df_historic_short_sma.index[number]]
            previous_historic_short_sma = df_historic_short_sma.loc[df_historic_short_sma.index[number - 1]]
        ...
        if df_historic_long_sma.size != 0:  # проверка на пустой DF
            historic_long_sma = df_historic_long_sma.loc[df_historic_long_sma.index[number]]
            previous_historic_long_sma = df_historic_long_sma.loc[df_historic_long_sma.index[number - 1]]
        ...
        historic_last_price = df_fin_close_prices[figi][number + 1]
        date = df_historic_long_sma.index[number]
...
def calc_historic_signals():
    ...
    df_fin_close_prices = pd.read_csv('Historic_close_prices.csv', sep=';', index_col=0)
    df_previous_sma_saving = pd.DataFrame(index=df_all_figi, columns=['previous_short_sma', 'previous_long_sma'])
    for n in tqdm(range(1), desc='calculating historic signals'):
        calc_one_historic_signal(-4, df_fin_close_prices)
...
df_all_figi = pd.read_csv('shares.csv', sep=';')
